function [data_WJ, data_WOJ] = CMWD_Processor_Count_Func(Subjects,FilePath)
    ...
    for FileIndex = 1:length(Subjects)
        ...
        while(1)
            ...
            if (sentinal==0)
            disp('Without jacket')
            DataICoPath = [FilePath Subject_woj '/ICo.mat'];
            DataICrossPath = [FilePath Subject_woj '/ICross.mat'];
            DataQCoPath = [FilePath Subject_woj '/QCo.mat'];
            DataQCrossPath = [FilePath Subject_woj '/QCross.mat'];
            
            save(DataICoPath,'DataICo','-v6');
            save(DataICrossPath,'DataICross','-v6');
            save(DataQCoPath,'DataQCo','-v6');
            save(DataQCrossPath,'DataQCross','-v6');
            
            elseif(sentinal==1)
            disp('with jacket')
            DataICoPath = [FilePath Subject_wj '/ICo.mat'];
            DataICrossPath = [FilePath Subject_wj '/ICross.mat'];
            DataQCoPath = [FilePath Subject_wj '/QCo.mat'];
            DataQCrossPath = [FilePath Subject_wj '/QCross.mat'];
            ...
            elseif(Target )
                ...
                if(PulseCounter == NumberOfPulses)
                    ...
                    for j = 1:(NumberOfPulses/(2*NoofPlusetoInteg))
                        ...
                        PluseIntegICross   = [PluseIntegICross ; mean(DataICross(startIndex:EndIndex,:))];

                        PluseIntegQCross   = [PluseIntegQCross ; mean(DataQCross(startIndex:EndIndex,:))];

                        PluseIntegICo      = [PluseIntegICo  ; mean(DataICo(startIndex:EndIndex,:))];

                        PluseIntegQCo      = [PluseIntegQCo ; mean(DataQCo(startIndex:EndIndex,:))];
                    end
